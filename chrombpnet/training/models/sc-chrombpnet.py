...
def sc_bpnet_model(filters, n_dil_layers, sequence_len, out_pred_len, sc_len):
    ...
    pos_sc_input = Input(shape=(sc_len, 1),name='scinput') 
    sc_dense = Dense(sc_len, name='sc_first_dense') (pos_sc_input)
    ...
    for i in range(1, n_dil_layers + 1):
        ...
        x = add([conv_x, x])
        sc_dense = Dense(x_len, name='sc_{}_dense'.format(layer_names[i-1]) ) (sc_dense)
        x = add([x, sc_dense])
    ...
    model=Model(inputs=[inp, pos_sc_input],outputs=[profile_out, count_out], name="model_wo_bias")
...
def getModelGivenModelOptionsAndWeightInits(args, model_params):   
    ...
    sc_len = int(model_params["sc_len"])


    bias_model = load_pretrained_bias(bias_model_path)
    bpnet_model_wo_bias = sc_bpnet_model(filters, n_dil_layers, sequence_len, out_pred_len)
    ...
    ## get wo bias output
    output_wo_bias=bpnet_model_wo_bias([inp, pos_sc_input])
    ...
    # instantiate keras Model with inputs and outputs
    model=Model(inputs=[inp, pos_sc_input],outputs=[profile_out, count_out])
