...
def read_parser():
        ...
        subparsers = parser.add_subparsers(help="Must be eithier 'pipeline', 'train', 'qc','convert', 'bias', 'prep', 'pred_bw', 'contribs_bw', 'modisco_motifs' ,'footprints', or 'snp_score'.", required=True, dest='cmd')
        ...
        convert_parser = subparsers.add_parser('convert', description="Convert input BAM/fragment/tagAlign file to appropriately shifted unstranded Bigwig")


        # convert parser
        convert_parser.add_argument('-g', '--genome', required=True, type=str, help="reference genome fasta file")
        ...
        convert_parser.add_argument('-c', '--chrom-sizes', type=str, required=True, help="Chrom sizes file")
        convert_parser.add_argument('-op', '--output-prefix', type=str, required=True, help="Output prefix (path/to/prefix)")
        convert_parser.add_argument('-d', '--data-type', required=True, type=str, choices=['ATAC', 'DNASE'], help="assay type")
        convert_parser.add_argument('-ps', '--plus-shift', type=int, default=None, help="Plus strand shift applied to reads. Estimated if not specified")
        convert_parser.add_argument('-ms', '--minus-shift', type=int, default=None, help="Minus strand shift applied to reads. Estimated if not specified")
        convert_parser.add_argument('--ATAC-ref-path', type=str, default=None, help="Path to ATAC reference motifs (chrombpnet/data/ATAC.ref.motifs.txt used by default)")
        convert_parser.add_argument('--DNASE-ref-path', type=str, default=None, help="Path to DNASE reference motifs (chrombpnet/data/DNASE.ref.motifs.txt used by default)")
        convert_parser.add_argument('--num-samples', type=int, default=10000, help="Number of reads to sample from BAM/fragment/tagAlign file for shift estimation")
